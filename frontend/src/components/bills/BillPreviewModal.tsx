...
export function BillPreviewModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center sm:p-4">
      ...
      {/* Container */}
      <div className="relative z-10 flex h-dvh w-full max-w-5xl flex-col overflow-hidden bg-background/95 sm:h-[calc(100dvh-2rem)] sm:max-h-[900px] sm:rounded-2xl sm:border sm:border-white/10 sm:shadow-2xl">
        {/* Header */}
        <div className="flex shrink-0 flex-col gap-3 border-b border-white/10 bg-background px-3 py-3 backdrop-blur-md sm:flex-row sm:items-center sm:justify-between sm:gap-4 sm:px-4">
        ...
        <div className="relative flex min-h-0 flex-1 items-center justify-center overflow-hidden">
          ...
          {bills.length > 1 && (
            <>
              <button
                onClick={prev}
                className="absolute left-2 z-10 flex h-8 w-8 items-center justify-center rounded-full border border-white/10 bg-background/60 text-foreground/70 backdrop-blur transition hover:border-foreground/25 hover:bg-background/70 hover:text-foreground sm:left-3 sm:h-9 sm:w-9"
              >
                <ChevronLeft className="h-5 w-5" />
              </button>
              <button
                onClick={next}
                className="absolute right-2 z-10 flex h-8 w-8 items-center justify-center rounded-full border border-white/10 bg-background/60 text-foreground/70 backdrop-blur transition hover:border-foreground/25 hover:bg-background/70 hover:text-foreground sm:right-3 sm:h-9 sm:w-9"
              >
        ...
        {/* Thumbnail strip */}
        {bills.length > 1 && (
          <div className="shrink-0 border-t border-white/8 bg-background/35 px-3 py-2.5 sm:px-4 sm:py-3">
            <div className="flex items-center gap-2 overflow-x-auto overscroll-x-contain pb-1">
              {bills.map((b, i) => (
                <button
                  key={b.id}
                  onClick={() => setIdx(i)}
                  className={`relative h-12 w-12 shrink-0 overflow-hidden rounded-lg border-2 transition-all duration-150 ${i === idx
